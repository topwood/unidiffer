...
Hunk.prototype.unifiedHeader = function () {
    var alen = this.alen === 1 ? '' : ',' + this.alen;
    var blen = this.blen === 1 ? '' : ',' + this.blen;
    // empty hunks show zeroith line (prior).  hunks with lines show first line number
    var afudg = this.alen === 0 ? 0 : 1;
    var bfudg = this.blen === 0 ? 0 : 1;
...
var ADDED = exports.ADDED = '+';
var REMOVED = exports.REMOVED = '-';
var UNMODIFIED = exports.UNMODIFIED = 's';
...
function lineChanges(change, select) {
    ...
    if (select === undefined) {
        ...
        var _i = nthIndexOf(v, '\n', len - 1, -select, true);
        lines = v.substring(_i + 1, len).split('\n');
    }
...
function makeHunks(changes, precontext, postcontext) {
    ...
    for (var ci = 0; ci < changes.length; ci++) {
        ...
        if (change.type === UNMODIFIED) {
            // add context
            var ctx_after = ci > 0 ? postcontext : 0; // context lines following previous change
            var ctx_before = ci < changes.length ? precontext : 0; // context lines preceding next change (iff there are more changes)
            var skip = Math.max(change.count - (ctx_after + ctx_before), 0);
            if (skip > 0) {
